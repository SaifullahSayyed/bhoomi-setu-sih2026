...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],   # localhost dev — tighten for production
    allow_methods=["*"],
    allow_headers=["*"],
)

# ---------------------------------------------------------------------------
# DATA LOADING (dataset generated by generate_synthetic_data.py)
# ---------------------------------------------------------------------------
...
def _load_all_parcels() -> list[dict]:
    ...
    # Community parcels (different schema)
...
# ---------------------------------------------------------------------------
# STARTUP — build Mirror Engine index once
# ---------------------------------------------------------------------------
...
class SealRequest(BaseModel):
    ...
    override_score: int | None = None  # admin override for demo (clearly labeled)
...
class VoteRequest(BaseModel):
    ...
    offline_batch: bool = False  # if True, simulates offline batch submission
...
# ---------------------------------------------------------------------------
# SEAL / CURTAIN ENDPOINTS (Priority 1c)
# ---------------------------------------------------------------------------
...
def seal_parcel(ulpin: str, request: SealRequest) -> dict:
    ...
    # Store full record off-chain (IPFS-equivalent)
    store = get_store()
    cid = store.put(p)

    # Build owner hash (hashing the primary owner's id_hash — never raw PII on-chain)
...
def mutate_parcel(ulpin: str, request: MutateRequest) -> dict:
    ...
    # Re-score with new owner context (simplified: score the existing parcel geometry)
    engine = get_engine()
...
# ---------------------------------------------------------------------------
# ASSURANCE POOL ENDPOINTS (Priority 2a)
# ---------------------------------------------------------------------------
...
def preview_premium(ulpin: str, declared_value: float = Query(..., gt=0)) -> dict:
    ...
    # Use bridge just for the formula calculation
    bridge = get_bridge()
...
# ---------------------------------------------------------------------------
# COMMUNITY TENURE ENDPOINTS (Priority 2b)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# PRIORITY 4 — ARCHITECTURE DEMOS (Honest Labels Applied)
# ---------------------------------------------------------------------------
